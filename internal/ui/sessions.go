...
func (a *App) viewSessions(w, h int) string {
	...
	head := s.Header.Render(fmt.Sprintf("%-12s %-14s %-22s %-10s %5s  %s",
		"ID", "NAME", "TITLE", "PLUGINS", "MSGS", "UPDATED"))
	...
	for _, role := range []string{"user", "assistant", "tool_call", "tool_result"} {
		n := ss.Counts[role]
		bar := miniBar(n, maxCount, 14, AccentAmber)
	...
	detail.WriteString(KV(s, "Cost", s.Accent.Render(fmt.Sprintf("$%.6f", ss.Cost)), kvw) + "\n")
